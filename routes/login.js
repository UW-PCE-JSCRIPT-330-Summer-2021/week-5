...
router.post("/password", isAuthorized, async (req, res, next) => {
  ...
  if (!password || password === '') {
    ...
    try {
      ...
      const success = await userDAO.updateUserPassword(req.userInfo._id, encryptedPW);
        ...
        if (!password || password === '') {
          ...
          bcrypt.compare(password, user.password, async (error, result) => {
            if (error || !result) {
              ...
              const token = jwt.sign({ email: user.email, _id: user._id, roles: user.roles }, privateKey);
              res.json({ token });
            }
          });
        }
